...
import { getEnquiryPageData } from "../../api/api";
...
export const EnquiriesPage = () => {
  ...
  const fetchData = async (dateRange) => {
    ...
    try {
      ...
      const data = await getEnquiryPageData(dbName, dateRange);
    ...
    <div className="bg-primary h-screen flex flex-col justify-between">
      ...
      <section className="min-h-[70%] rounded-t-[4vh] bg-secondary p-4 ">
        <div className="text-center mt-2">
          ...
          <div className="flex gap-2 border-2 mt-3 border-black rounded-md ">
            <p className="w-1/2 text-center text-primary p-2 font-semibold">
              Course Name
            </p>
            <p className="w-1/2 text-center text-primary p-2 font-semibold border-l-2 border-black">
              Course Enquiries
            </p>
          </div>
